...
describe('Response Content-Type', () => {
  test('regular json, application/json', () => request(server)
    .get('/api/content?type=json')
    .expect('Content-Type', /application\/json/)
  )

  test('yml to json -> application/json', () => request(server)
    .get('/api/content?type=json2')
    .expect('Content-Type', /application\/json/)
  )

  test('text/html', () => request(server)
...
describe('Matcher modifiers', () => {
  test('.has: string value', () => request(server)
    .get('/api/modifier')
    .set('x-has-single', 'anything')
    .expect(201)
  )

  test('.has: array value', () => request(server)
    .get('/api/modifier')
  ...
  test('.equal', () => request(server)
    .post('/api/modifier')
    .send({one: 1})
    .expect(204)
  )

  test('.equal, extra content', () => request(server)
    .post('/api/modifier')
    .send({one: 1, two: 2})
    .expect(404)
  )
